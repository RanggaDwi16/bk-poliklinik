...
function tambahJadwalPeriksa__($data)
{
    try {
        ...
        // Check if the jadwal periksa already exists for the same dokter on the same day
        $query_existing = "SELECT * FROM jadwal_periksa WHERE id_dokter = $id_dokter AND hari = '$hari'";
        $existing_data = mysqli_query($conn, $query_existing);

        if (mysqli_num_rows($existing_data) > 0) {
            return -2; // Return -2 if the jadwal periksa already exists for the same dokter on the same day
        }
...
function updateJadwalBedaHari($data, $id)
{
    ...
    $id_dokter = mysqli_real_escape_string($conn, $data["id_dokter"]);
    $hari = mysqli_real_escape_string($conn, $data["hari"]);
    $jam_mulai = mysqli_real_escape_string($conn, $data["jam_mulai"]);
    $jam_selesai = mysqli_real_escape_string($conn, $data["jam_selesai"]);
    ...
    try {
        // Jika jadwal akan diaktifkan, pastikan tidak ada jadwal aktif di hari yang sama
        if ($aktif === 'Y') {
            $queryCheck = "SELECT * FROM jadwal_periksa 
            ...
            if (mysqli_num_rows($result) > 0) {
                // Nonaktifkan jadwal aktif di hari yang sama
                $queryDeactivate = "UPDATE jadwal_periksa 
                                    SET aktif = 'T' 
                                    WHERE id_dokter = '$id_dokter' 
                                    AND hari = '$hari' 
                                    AND aktif = 'Y'";
                mysqli_query($conn, $queryDeactivate);
            }
        }
        ...
        if (mysqli_query($conn, $queryUpdate)) {
            return mysqli_affected_rows($conn); // Berhasil mengupdate
        } else {
            echo "Error updating record: " . mysqli_error($conn);
            return -1; // Error saat mengupdate
        }
    } catch (Exception $e) {
